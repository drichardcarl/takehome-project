...
interface TimelineState {
  ...
  getSceneAtFrame: (frame: number) => { scene: Scene; localFrame: number } | null;
}

// Helper function to generate random colors
const generateRandomColor = (): string => {
  const colors = [
    '#F65937', '#379EF6', '#1FBD5F', '#F6A337', '#9B37F6',
    '#F6379B', '#37F6A3', '#F6E337', '#37A3F6', '#F63737'
  ];
...
export const useTimelineStore = create<TimelineState>((set, get) => ({
  ...
  addScene: () => {
    ...
    const command: Command = {
      execute: () => {
        ...
        const totalFrames = recalculatedScenes.reduce((sum, scene) => sum + scene.scene_length, 0);
        set({ 
  ...
  resizeScene: (sceneIndex: number, newLength: number) => {
    ...
    const command: Command = {
      execute: () => {
        const updatedScenes = [...scenes];
        updatedScenes[sceneIndex] = { ...updatedScenes[sceneIndex], scene_length: newLength };
        const recalculatedScenes = recalculateScenePositions(updatedScenes);
        const totalFrames = recalculatedScenes.reduce((sum, scene) => sum + scene.scene_length, 0);
        set({ 
          scenes: recalculatedScenes, 
      ...
      undo: () => {
        const updatedScenes = [...scenes];
        updatedScenes[sceneIndex] = { ...updatedScenes[sceneIndex], scene_length: oldLength };
        const recalculatedScenes = recalculateScenePositions(updatedScenes);
        const totalFrames = recalculatedScenes.reduce((sum, scene) => sum + scene.scene_length, 0);
        set({ 
          scenes: recalculatedScenes, 
  ...
  setCurrentFrame: (frame: number) => set(state => ({
    playback: { ...state.playback, currentFrame: Math.max(0, Math.min(frame, state.totalFrames - 1)) }
  })),

  setPlaybackState: (isPlaying: boolean) => set(state => ({
  ...
  getSceneAtFrame: (frame: number) => {
    ...
    const scene = scenes.find(s => 
      frame >= s.start_frame && frame < s.start_frame + s.scene_length
    );
